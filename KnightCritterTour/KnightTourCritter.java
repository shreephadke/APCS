...
public class KnightTourCritter extends KnightCritter
{
    ...
    // algorithm.
    public int[][] accessGrid = { { 2, 3, 4, 4, 4, 4, 3, 2 }, { 3, 4, 6, 6, 6, 6, 4, 3 },
    ...
    public Location getBestMove( ArrayList<Location> locs )
    {
        ...
        int x = accessGrid[row][col];
        for ( int i = 0; i < locs.size(); i++ )
        {
            if ( accessGrid[locs.get( i ).getRow()][locs.get( i ).getCol()] < x )
            {
                x = accessGrid[locs.get( i ).getRow()][locs.get( i ).getCol()];
                bestMove = locs.get( i );
            }
    ...
    public void updateAccess( Location loc )
    {
        ...
        {
            int temp1 = list.get( i ).getRow();
            int temp2 = list.get( i ).getCol();

            accessGrid[temp1][temp2]--;
        }

        accessGrid[row][col] = -1; // not in grid
    ...
    public Location selectMoveLocation( ArrayList<Location> locs )
    {
        ...
        Location location = getBestMove( locs );
